...
class ScenarioResult:
    ...
    def set_collision_info(self, collision_info):
        if collision_info.has_collided:
            if 'person' in collision_info.object_name or 'bird' in collision_info.object_name:
                self.collision = 1
...
class TestFramework:
    def __init__(self, pop_size, generations, mutation_rate, host_ip, scenario_topic, result_save_path='/home/yao/Documents/random_result'):
        self.pop_size = pop_size
        self.generations = generations
        self.mutation_rate = mutation_rate
        # create multiple airsim connections for different threads to avoid IOLOOP error
        self.env = AirSimEnv(host_ip=host_ip)
        self.env_cb = AirSimEnv(host_ip=host_ip)
    ...
    def initial_population(self):
        scenarios = [sample_scenario() for _ in range(self.pop_size)]
        return scenarios
