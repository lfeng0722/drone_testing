...
from control.ardupilot_control import ArdupilotController
...
class Controller:
    ...
    def cb_local_target(self, cmd_msg):
        ...
        if self.enable:
            ...
            pose = ScenarioPose(position.x, position.y, -position.z, 0)
            position_to_ardu = [position.x, position.y, position.z]
            ...
            # scenario_pose = Pose(position.x, position.y, position.z, math.degrees(yaw))
                self.drone_controller.move_to_local(position_to_ardu)
...
if __name__ == "__main__":
    ...
    drone_name = rospy.get_param('~drone_name', 'naga')
    ...
    ardupilot_control = ArdupilotController(drone_name)
    
    time.sleep(3)
    # print(ardupilot_control.mode)
    # time.sleep(60)
    # ardupilot_control.takeoff()
    # time.sleep(10)
    # ardupilot_control.move_to_local([10, 10, 15])
    # time.sleep(5)
    # ardupilot_control.land()

    controller = Controller(ardupilot_control, local_target_topic, global_target_topic, scenario_topic)
