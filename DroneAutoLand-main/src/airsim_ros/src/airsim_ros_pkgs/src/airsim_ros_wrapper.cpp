...
//        std::cout << "get origin ego point" << std::endl;
...
//    std::cout << "set state cb" << std::endl;
...
void AirsimROSWrapper::create_ros_pubs_from_settings_json()
{
    ...
    cam_pose_global_pub_vec_.clear();
    camera_info_msg_vec_.clear();
    camera_pose_msg_vec_.clear();
    camera_pose_global_msg_vec_.clear();
        ...
        for (auto& curr_camera_elem : vehicle_setting->cameras) {
            ...
            for (const auto& curr_capture_elem : camera_setting.capture_settings) {
                ...
                if (!(std::isnan(capture_setting.fov_degrees))) {
                    ...
                    cam_pose_pub_vec_.push_back(nh_private_.advertise<geometry_msgs::PoseStamped>(cam_image_topic + "/camera_pose", 10));
                    cam_pose_global_pub_vec_.push_back(nh_private_.advertise<geometry_msgs::PoseStamped>(cam_image_topic + "/camera_pose_global", 10));
...
bool AirsimROSWrapper::takeoff_srv_cb(airsim_ros_pkgs::Takeoff::Request& request, airsim_ros_pkgs::Takeoff::Response& response, const std::string& vehicle_name)
{
    ...
    if (request.waitOnLastTask)
        get_multirotor_client()->takeoffAsync(20, vehicle_name)->waitOnLastTask(); // todo value for timeout_sec?
    else
...
bool AirsimROSWrapper::takeoff_all_srv_cb(airsim_ros_pkgs::Takeoff::Request& request, airsim_ros_pkgs::Takeoff::Response& response)
{
    ...
    if (request.waitOnLastTask)
        for (const auto& vehicle_name_ptr_pair : vehicle_name_ptr_map_)
            get_multirotor_client()->takeoffAsync(20, vehicle_name_ptr_pair.first)->waitOnLastTask(); // todo value for timeout_sec?
    else
        for (const auto& vehicle_name_ptr_pair : vehicle_name_ptr_map_)
            get_multirotor_client()->takeoffAsync(20, vehicle_name_ptr_pair.first);
...
bool AirsimROSWrapper::land_all_srv_cb(airsim_ros_pkgs::Land::Request& request, airsim_ros_pkgs::Land::Response& response)
{
    ...
    if (request.waitOnLastTask)
        for (const auto& vehicle_name_ptr_pair : vehicle_name_ptr_map_)
            get_multirotor_client()->landAsync(60, vehicle_name_ptr_pair.first)->waitOnLastTask();
    else
        for (const auto& vehicle_name_ptr_pair : vehicle_name_ptr_map_)
            get_multirotor_client()->landAsync(60, vehicle_name_ptr_pair.first);
...
//        std::cout << "try to get sim time" << std::endl;
        ...
        if (publish_clock_) {
//            std::cout << "publish clock" << std::endl;
...
void AirsimROSWrapper::process_and_publish_img_response(const std::vector<ImageResponse>& img_response_vec, const int img_response_idx, const std::string& vehicle_name)
{
    ...
    for (const auto& curr_img_response : img_response_vec) {
        ...
        camera_pose_msg_vec_[img_response_idx_internal].header.stamp = airsim_timestamp_to_ros(curr_img_response.time_stamp);
        cam_pose_pub_vec_[img_response_idx_internal].publish(camera_pose_msg_vec_[img_response_idx_internal]);
...
void AirsimROSWrapper::publish_camera_tf(const ImageResponse& img_response, const ros::Time& ros_time, const std::string& frame_id, const std::string& child_frame_id)
{
    ...
    // generate camera optical pose message
    geometry_msgs::PoseStamped cam_optical_pose_msg;
    cam_optical_pose_msg.header.stamp = airsim_timestamp_to_ros(img_response.time_stamp);
    cam_optical_pose_msg.header.frame_id = frame_id;
    cam_optical_pose_msg.pose.position.x = cam_tf_body_msg.transform.translation.x;
    cam_optical_pose_msg.pose.position.y = cam_tf_body_msg.transform.translation.y;
    cam_optical_pose_msg.pose.position.z = cam_tf_body_msg.transform.translation.z;
    cam_optical_pose_msg.pose.orientation.x = cam_tf_body_msg.transform.rotation.x;
    cam_optical_pose_msg.pose.orientation.y = cam_tf_body_msg.transform.rotation.y;
    cam_optical_pose_msg.pose.orientation.z = cam_tf_body_msg.transform.rotation.z;
    cam_optical_pose_msg.pose.orientation.w = cam_tf_body_msg.transform.rotation.w;

    camera_pose_msg_vec_.push_back(cam_optical_pose_msg);

    // get the global pose of the camera relative to world frame
