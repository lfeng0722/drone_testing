            ...
            if self.drone_status == 'in searching':
                if len(self.detected_markers) != 0:
                    ...
                    if self.current_landing_location is None and detected_marker.confidence >= 0.8 and detected_marker.id == 0:
                        ...
                        landing_location = [detected_marker.pose.position.x, detected_marker.pose.position.y, self.landing_height]
                        rospy.loginfo("set landing target: " + str([landing_location[0], landing_location[1], self.landing_height]))
                        landing_location_pose = ScenarioPose()
                        landing_location_pose.x = float(detected_marker.pose.position.x)
                        landing_location_pose.y = float(detected_marker.pose.position.y)
                        landing_location_pose.z = float(-self.landing_height)
                        landing_location_pose.yaw = float(0)
                        ...
                        self.global_location_pub.publish(landing_location_pose)
                    ...
                    #     self.landing_location_pub.publish(create_path(landing_location))
                        ...
                        location = [detected_marker.pose.position.x, detected_marker.pose.position.y, -1]
                        if distance_2d(self.current_landing_location, location[:2]) > 0.7:
                            self.current_landing_location = location[:2]
                            rospy.loginfo("update landing target: " + str([location[0], location[1], -1]))
                            landing_location_pose = ScenarioPose()
                            # landing_location_pose = ScenarioPose()
                            landing_location_pose.x = float(detected_marker.pose.position.x)
                            landing_location_pose.y = float(detected_marker.pose.position.y)
                ...
                if self.current_landing_location and distance_2d(self.current_landing_location,
                    [self.drone_position.y, self.drone_position.x]) < 1 and abs(-self.drone_position.z - self.landing_height) < 0.5:
