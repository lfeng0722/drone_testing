...
def sample_marker(marker_pose, type='tp'):
    ...
    return marker
#map_0 is court
def sample_pose(type=0, map_name='map_1'):
    if map_name == 'map_0' and ACTOR_TYPE_DICT[type] != 'bird':
        random_pos_x = random.uniform(-10, 10)  # Just an example range
        random_pos_y = random.uniform(-35, 35)
        random_pos_z = 0.1
    elif map_name == 'map_1' and ACTOR_TYPE_DICT[type] != 'bird':
        random_pos_x = random.uniform(-8, 8)  # Just an example range
        random_pos_y = random.uniform(-50, 50)
        random_pos_z = 0.01
    elif map_name == 'map_2' and ACTOR_TYPE_DICT[type] != 'bird':
        random_pos_x = random.uniform(-8, 8)  # Just an example range
        random_pos_y = random.uniform(-50, 50)
        random_pos_z = 0.01
    ...
    pose = Pose(random_pos_x, random_pos_y, random_pos_z, random_yaw_angle)
    return pose


def sample_pose_on_circle(gps_pose, radius, actor_type=0):
...
def sample_marker_in_circle(gps_pose, radius, type='tp'):
    ...
    marker_x, marker_y, marker_z = sample_pose_on_circle(gps_pose, radius, -1)
...
def sample_actor(gps_pose, radius=5, dynamic=False):
    ...
    start_x, start_y, start_z = sample_pose_on_circle(gps_pose, radius, actor_type)
