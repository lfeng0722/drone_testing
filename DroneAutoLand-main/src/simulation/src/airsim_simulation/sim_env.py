...
try:
    ...
    from configuration import map_config
...
class AirSimEnv():
    ...
    def __init__(self, sim_mode='drone', host_ip='localhost', map_name="court") -> None:
        # define airsim API client
        self.sim_mode = sim_mode
        self.map_config = map_config[map_name]
        if sim_mode == 'cv':
        ...
        # save all dynamic objects' initial poses and whether objects have been used in the current scenario
        self.dynamic_objects_pool = self.get_objects_pool()


        # dynaimc scenario objects in the current scenario
        self.scenario_objects = []

    def get_objects_pool(self):
    ...
    def set_marker(self, scenario_pose, marker_name='cube_marker0_13'):
    ...
    def reset_env(self):
        ...
        # reset markers
        # markers = []
        # markers.append('cube_marker{}'.format())
        markers = ['Cube_marker{}'.format(i) for i in range(3)]
        # markers.append('cube_marker0')
    ...
    '''
    Initialize dynamic objects in the current scenario
    '''
    def add_npc(self, npc_type, scenario_pose):
        ...
        # bp_name = self.map_config["bp"][npc_type]
        # npc_name = 'npc_{}'.format(npc_type)
        # try:
    ...
    def move_npc_forward(self, npc_name, speed=0.5):
        ...
        new_x = npc_position.position.x_val + 2
    ...
    def move_npc_right(self, npc_name, speed):
        npc_position = self.get_pose(npc_name)
        new_y = npc_position.position.y_val - 2
