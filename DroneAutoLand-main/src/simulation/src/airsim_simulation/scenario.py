...
def person_near_marker_scenario():
    gps_pose = Pose(-10, 0, 0, 0)
    tp_marker = Marker(0, MARKER_MATERIAL_DICT[0], Pose(-10, 0, 0))
    weather = Weather(*([0] * 9))
    time = Time(0.5, 0.5)
    actors = [Actor(0, Pose(-8, 0, 0), Pose(-8, 0, 0), 0)]
    scenario = Scenario()
    scenario.init(tp_marker, [], Pose(0, 0, 0), gps_pose, 10, actors, weather, time)
    return scenario

def sample_test_scenario():
    gps_pose = Pose(0, 10, -10, 0)
    radius = 7.5
    tp_marker = sample_marker_in_circle(gps_pose, radius, 'tp')
    tp_marker.pose.z =-1
    # print('1111111111111',tp_marker.pose)
    tp_marker.id = 0
    fp_markers= []
    ...
    actors = []
    for _ in range(1):
        start_x, start_y, start_z = sample_pose_on_circle(tp_marker.pose, 3)
